...
<div id="readbookcon" class="content">
    ...
    <div class="m2" id="chapter" style='display:block'>
        ...
        <div class="listNotice">
            ...
            <div id="chapterlist">
                <?php if(is_array($chapterData)): foreach($chapterData as $key=>$v): ?><a href="/Home/Book/reading.html?bookId=<?php echo ($bookDetail['id']); ?>&chapterId=<?php echo ($v["id"]); ?>">
        ...
        <div class="paging" id="selpageindex">
            <ul>
                ...
                <?php if($nowpage-10 > 0): ?>/Home/Book/chapter?bookId=<?php echo ($bookId); ?>&page=<?php echo ($nowpage-10); ?>
                   <?php else: ?>#<?php endif; ?>
                        "><<</a> </li>
                <li data-value="2" style="font-size: 12px"><a href="
                <?php if($nowpage-1 > 0): ?>/Home/Book/chapter?bookId=<?php echo ($bookId); ?>&page=<?php echo ($nowpage-1); ?>
                    <?php else: ?>#<?php endif; ?>
                    "><</a></li>
                <li>
                    <select id="selPage">
                    ...
                    <script>
                        $('#selPage').change(function(){
                            location.href='/Home/Book/chapter?bookId=<?php echo ($bookId); ?>&page='+$(this).val();
                        });
                    </script>
                </li>
                <li data-value="3" style="font-size: 12px"><a href="
                <?php if($nowpage < $pageCount): ?>/Home/Book/chapter?bookId=<?php echo ($bookId); ?>&page=<?php echo ($nowpage+1); ?>
                    <?php else: ?>#<?php endif; ?>
                    ">></a></li>
                <li data-value="4" style="font-size: 12px"><a href="
                <?php if($nowpage+10 < $pageCount): ?>/Home/Book/chapter?bookId=<?php echo ($bookId); ?>&page=<?php echo ($nowpage+10); ?>
                    <?php else: ?>#<?php endif; ?>
                    ">>></a></li>
            </ul>
        </div>
